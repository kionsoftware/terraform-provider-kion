...
def import_compliance_standards():
    ...
    Handles the full process to import compliance standards
    ...
    if STANDARDS:
        ...
        for s in STANDARDS:
            ...
            # Here, we specifically check if a standard is system-managed.
            # Assuming system_managed and ct_managed both being True indicates
            # a standard is system-managed. Adjust the logic if the criteria differ.
            if s['system_managed']:
                if not ARGS.clone_system_managed:
                    print(
                        "Skipping System-managed Compliance Standard - %s" % s['name'])
                    continue
                else:
                    system_managed = True
